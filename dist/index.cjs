...
function isEmpty(path3) {
  const files = import_fs.default.readdirSync(path3);
...
function emptyDir(dir) {
  ...
  for (const file of import_fs.default.readdirSync(dir)) {
    ...
    import_fs.default.rmSync(import_path.default.resolve(dir, file), { recursive: true, force: true });
  }
}
...
var TEMPLATES = FRAMEWORKS.map(
  (framework) => framework.variants && framework.variants.map((value) => value.name) || [framework.name]
).reduce((a, b) => a.concat(b), []);
var defaultTargetDir = "msfs-glasscockpit";
async function init() {
  const argTargetDir = formatTargetDir(argv._[0]);
  const argTemplate = argv.template || argv.t;
  let targetDir = argTargetDir || defaultTargetDir;
  const getProjectName = () => targetDir === "." ? import_path2.default.basename(import_path2.default.resolve()) : targetDir;
  let result;
  try {
    result = await (0, import_prompts.default)(
      [
        ...
        {
          type: () => !import_fs2.default.existsSync(targetDir) || isEmpty(targetDir) ? null : "select",
          name: "overwrite",
        ...
        },
        {
          ...
          name: "framework",
          message: typeof argTemplate === "string" && !TEMPLATES.includes(argTemplate) ? reset(`Invalid template: ${argTemplate}. Please choose from below:`) : reset("Select a framework:"),
          initial: 0,
          choices: FRAMEWORKS.map((framework2) => ({
            title: framework2.color(framework2.display || framework2.name),
            value: framework2
          }))
        },
        {
          ...
          choices: (framework2) => framework2.variants.map((variant2) => ({
            title: variant2.color(variant2.display || variant2.name),
            value: variant2.name
          }))
        },
        {
...
Scaffolding project in ${root}...`);
  const templateDir = import_path2.default.resolve(__dirname, "../templates", `template-${template}`);
  ...
  const cdProjectName = import_path2.default.relative(cwd, root);
  console.log(`
Done. Now run:
`);
  if (root !== cwd) {
